...
\item Given the forward pass computation:
  \begin{align}
    \mS &= \mQ \mK^\top \label{eq:s_eq}\\
    \mP &= \text{SoftMax}(\mS) \label{eq:softmax_eq}\\
...
\item Goal: Compute the input gradients
  \begin{equation*}
    \frac{\partial L}{\partial \mQ}, \quad \frac{\partial L}{\partial \mK}, \quad \text{and} \quad \frac{\partial L}{\partial \mV}
  \end{equation*}
\item We will compute the gradients w.r.t. $\mQ$, $\mK$, and $\mV$ through autodiff reverse mode. For illustration, we use $T=2$, $d=1$ for computational graphs.
\end{itemize}
\end{frame}


\begin{frame}[allowframebreaks]
\frametitle{Gradient w.r.t. V: ($\frac{\partial L}{\partial \mV}$)}
\begin{itemize}
...
\vspace{1cm}
\begin{itemize}
\item Extending the derivation for each element of $V$, we get:
  \begin{equation*}
...
\begin{itemize}
\item Similarly, we can get the gradient w.r.t. P:
  \begin{equation*}
...
\begin{frame}[allowframebreaks]
\frametitle{Gradient w.r.t. S ($\frac{\partial L}{\partial \mS}$)}
\begin{itemize}
\item Now we need 
  \begin{equation*}
    \frac{\partial L}{\partial \mS} \quad \text { from } \quad \frac{\partial L}{\partial \mP} \quad \text { using } \quad \eqref{eq:softmax_eq}
  \end{equation*}
...
\item Node definitions:
  \begin{align*}
    ...
    v_5 &= v_1 + v_2 = e^{S_{11}} + e^{S_{12}} \quad \\
    v_6 &= v_3 + v_4 = e^{S_{21}} + e^{S_{22}} \quad \\
    P_{11} &= \frac{v_1}{v_5}, \quad P_{12} = \frac{v_2}{v_5}, \quad P_{21} = \frac{v_3}{v_6}, \quad P_{22} = \frac{v_4}{v_6}
  \end{align*}
\end{itemize}

\begin{itemize}
\item From the computational graph, $S_{11}$ affects $L$ through both $P_{11}$ and $P_{12}$:
  \begin{equation*}
  ...
  \end{equation*}
\item Therefore, we need to compute 

  \begin{equation*}
    \frac{\partial P_{11}}{\partial S_{11}} \quad \text{and} \quad \frac{\partial P_{12}}{\partial S_{11}}
  \end{equation*}
\end{itemize}

\begin{itemize}
\item Path 1: $S_{11} \to v_1 \to P_{11}$
  \begin{align*}
...
\begin{itemize}
\item Path 2: $S_{11} \to v_1 \to v_5 \to P_{12}$
  \begin{align*}
...
\begin{itemize}
\item Substituting back into the total gradient:
  \begin{align*}
    ...
    &= P_{11} \left( \frac{\partial L}{\partial P_{11}} (1 - P_{11}) - \frac{\partial L}{\partial P_{12}} P_{12} \right) \\
    &= P_{11} \left( \frac{\partial L}{\partial P_{11}} - P_{11}\frac{\partial L}{\partial P_{11}} - P_{12}\frac{\partial L}{\partial P_{12}} \right) \\
    &= P_{11} \left( \frac{\partial L}{\partial P_{11}} - \sum_{k=1}^2 P_{1k}\frac{\partial L}{\partial P_{1k}} \right)
  \end{align*}
\end{itemize}

\begin{itemize}
\item Generalizing to any $S_{ij}$, we get the general SoftMax gradient formula:
  \begin{equation*}
...
\begin{frame}[allowframebreaks]
\frametitle{Gradient w.r.t. K ($\frac{\partial L}{\partial \mK}$)}
\begin{itemize}
\item From \eqref{eq:s_eq}, we have $S_{ij} = \sum_{\ell=1}^d Q_{i\ell} K_{j\ell}$
\end{itemize}
...
\begin{itemize}
\item From the computational graph, $K_{11}$ affects $L$ through both $S_{11}$ and $S_{21}$:
  \begin{equation*}
  ...
  \end{equation*}
\item Since $S_{11} = Q_{11} K_{11}$ and $S_{21} = Q_{21} K_{11}$:
  \begin{align*}
...
\begin{itemize}
\item Generalizing to any $K_{j\ell}$:
  \begin{equation*}
...
\begin{frame}[allowframebreaks]
\frametitle{Gradient w.r.t. Q: ($\frac{\partial L}{\partial \mQ}$)}
\begin{itemize}
\item Similarly, we can get the gradient w.r.t. Q:
  \begin{align*}
...
\begin{frame}[allowframebreaks]
\frametitle{Assumptions for Standard Attention Backward Pass}
\begin{itemize}
\item The intermediate matrices $\mP$, $\mS$, the output $\mO$, and the output gradient
\begin{equation*}
  \frac{\partial L}{\partial \mO}
\end{equation*}
are stored in HBM
\item The operations are done in blocks of size $|B|$, where $B$ denotes a block index set
\item Memory constraint: $d \leq M \leq Td$ where $M$ is the SRAM size
\end{itemize}
\end{frame}
...
\begin{itemize}
\item For computing $|B|$ rows of 
\begin{equation*}
  \frac{\partial L}{\partial \mV}
\end{equation*}
: we need 3 blocks in the main memory

  \begin{enumerate}
  \item Block 1:
  \begin{equation*}
    \frac{\partial L}{\partial \mV_{B,:}} \in \R^{|B| \times d}
  \end{equation*}
  \item Block 2:
  \begin{equation*}
    \mP_{:,B} \in \R^{|B| \times |B|} \text{ (loaded from HBM)}
  \end{equation*}
  \item Block 3:
  \begin{equation*}
    \frac{\partial L}{\partial \mO_{B,:}} \in \R^{|B| \times d} \text{ (loaded from HBM)}
  \end{equation*}
  \end{enumerate}
  From constraints 
  \begin{equation*}
    |B|d \leq \frac{M}{3} \text{ and } |B|^2 \leq \frac{M}{3}
  \end{equation*}
  we get:
  \begin{equation*}
    |B| = \min\left(\lfloor \frac{M}{3d} \rfloor, \lfloor \sqrt{\frac{M}{3}} \rfloor\right)
  \end{equation*}
\end{itemize}
\end{frame}
...
Let $N_b = \lceil \frac{T}{|B|} \rceil$ be the number of blocks. We have the following algorithm:

\begin{itemize}
\item Step 1: Gradient w.r.t. V
  \begin{itemize}
...
\vspace{1cm}
\item Step 3: Gradient w.r.t. S
  \begin{itemize}
  \item For $i = 1, \ldots, N_b$
    \begin{itemize}
    \item Compute row-wise sum: $\text{sum}_{I_i} = \sum_{j=1}^T \frac{\partial L}{\partial P_{ij}} P_{ij}$ (element-wise product and sum)
...
\item Steps 1, 2, 4, 5:
  \begin{equation*}
    O(N_b) \times O(N_b) \times O(|B|^2 \times d) = O(T^2 \times d)
  \end{equation*}
\item Step 3:
  \begin{equation*}
    O(N_b) \times O(N_b) \times O(|B|^2) = O(T^2 \times |B|^2)
  \end{equation*}
...
\begin{itemize}
\item From algorithm, the I/O bottleneck is to read $\mP$ and write 
\begin{equation*}
